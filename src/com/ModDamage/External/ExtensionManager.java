...
import com.ModDamage.Backend.Minecraft.Properties.BlockProps;
import com.ModDamage.Backend.Minecraft.Properties.ChunkProps;
import com.ModDamage.Backend.Minecraft.Properties.CreatureProps;
import com.ModDamage.Backend.Minecraft.Properties.EntityProps;
import com.ModDamage.Backend.Minecraft.Properties.EquipmentProps;
import com.ModDamage.Backend.Minecraft.Properties.InventoryProps;
import com.ModDamage.Backend.Minecraft.Properties.ItemProps;
import com.ModDamage.Backend.Minecraft.Properties.LocationProps;
import com.ModDamage.Backend.Minecraft.Properties.MaterialProps;
import com.ModDamage.Backend.Minecraft.Properties.MiscProps;
import com.ModDamage.Backend.Minecraft.Properties.OfflinePlayerProps;
import com.ModDamage.Backend.Minecraft.Properties.PlayerProps;
import com.ModDamage.Backend.Minecraft.Properties.ScoreboardProps;
import com.ModDamage.Backend.Minecraft.Properties.ServerProps;
import com.ModDamage.Backend.Minecraft.Properties.WorldProps;
import com.ModDamage.Backend.Minecraft.Variables.PlayerNamed;
import com.ModDamage.Backend.Minecraft.Variables.TagValue;
import com.ModDamage.Backend.Minecraft.Variables.Transformers;
import com.ModDamage.Backend.Minecraft.Variables.WorldNamed;
...
import com.ModDamage.Routines.Expressions.NestedExpression;
import com.ModDamage.Routines.Expressions.NewVectorFunction;
...
public class ExtensionManager
{
	...
	{
		String prefix = ModDamage.getConfiguration().getLog().logPrepend();
		if (registeredPlugins.isEmpty())
		{
			ModDamageLogger.info_verbose(prefix + "Extensions: No extensions found.");
		} else {
			ModDamageLogger.info_verbose(prefix + "Extensions: Loading...");
			boolean anyFailed = false;
			//TODO include extension load count?
			for(ModDamageExtension pluginExtension : registeredPlugins)
			{
				...
				try {
					...
					if (!currentFailed)
						ModDamageLogger.info_verbose(prefix + "Extension '" + description.getName() + " v" + description.getVersion()+ "' has been reloaded successfully.");
					else
						ModDamageLogger.error(prefix + "Extension '" + description.getName() + " v" + description.getVersion() + "' failed to reload.");
				} catch (Throwable e) { 
					ModDamageLogger.error(prefix + "An extension failed to load:" + pluginExtension.toString()); 
					e.printStackTrace();
				}
			}
			if (!anyFailed)	ModDamageLogger.constant(prefix + "Extensions: Successfully loaded all extensions.");
			else ModDamageLogger.warning_strong(prefix + "Extensions: Some extensions failed to load.");
		}
	}
	...
	private static void reloadModDamageRoutines()
	{
		...
		NumberExpression.registerAllNumbers();
		StringExpression.register();
		NestedExpression.register();
		TagValue.register();
		...
		try {
			...
			ModDamageLogger.info("Vault not found: "+e.getMessage());
		}
		...
		try {
			...
			ModDamageLogger.info("TabAPI not found: "+e.getMessage());
			}
