package com.ModDamage.Backend.Minecraft.Events.EventInfo;
...
//	}
	...
	public Object get(int i)
	{
		if (i - start >= objects.length) {
			ModDamageLogger.error("Bad index of "+i+" / "+start+"+"+objects.length);
			return null;
		}
	...
	public <T> T get(Class<T> cls, int i)
	{
		...
		ModDamageLogger.error("Bad cast of "+o+" to "+cls.getSimpleName());
		return null;
	}
	...
	public <T> void set(Class<T> cls, int i, T obj)
	{
		if (i >= start)
			...
			ModDamageLogger.error("Setting bad index "+i+" to "+obj+" type "+cls.getSimpleName());
	}
