...
import com.ModDamage.Backend.Minecraft.Events.Repeat.RepeatInfo.RepeatData;
...
public class Repeat extends MDEvent implements ScriptLineHandler
{
	...
	public ScriptLineHandler handleLine(ScriptLine line, boolean hasChildren)
	{
		...
		if (parts.length == 1 || parts[1].equalsIgnoreCase("entity"))
			...
			ModDamageLogger.error("Illegal repeat type: "+parts[1]);
			return null;
		}
		...
		ModDamageLogger.info("Repeat ["+repeat.name+" "+repeat.repeatType.getSimpleName()+"]");
...
//		{
//			ModDamageLogger.error("Expected List, got "+repeats.getClass().getSimpleName()+"for Repeat event");
//			return;
//		}
//
...
//	}
	...
	static class RepeatInfo<T>
	{
		...
		public class RepeatData implements Runnable
		{
			...
			private void start()
			{
				...
				else ModDamageLogger.warning_strong("Unable to start repeat task!");
			}
	...
	public static void start(String name, Object it, int delay, int count) {
		...
		if (info == null) 
		{
			ModDamageLogger.error("No Repeat named "+name);
			return;
		}
